...
El Frontend incluye las siguientes pantallas: Login (inicio de sesión), Dashboard (panel principal), Productos (visualización y control de productos, resaltando bajo stock), Ventas (registro de ventas simples) y Reportes (visualización de productos con stock mínimo). Las vistas y acciones disponibles se habilitan dinámicamente según el rol del usuario autenticado. El consumo de la API se realiza mediante clases de servicio dedicadas, evitando llamadas directas desde los controladores y manteniendo un código más limpio y mantenible, en la clase de servicio se debe modfificar las URL en las llamadas del API cambiandole el puerto en la que se esta ejecutando el API localmente para poder tener comunicación con el Backend, si no se hace tendremos problemas de comunicación por ejemplo tenemos la URL https://localhost:72/api/Productos la sustituiremos por https:/(dirección en la que se ejecuta con el puerto)/api/Productos dejando los ultimos segmento tal cual ya que ese no cambia, tenemos que tener en cuenta que la solucion del Backend tiene que estar en ejecución.

El sistema implementa validaciones utilizando DataAnnotations en los modelos, control de datos obligatorios, manejo estructurado de errores y respuestas HTTP adecuadas (200, 400, 500). Se incluyen operaciones GET, POST, PUT y DELETE para demostración del manejo completo de endpoints REST aunque no todas estan implementadas en el Frontend.

Entre las principales decisiones técnicas esta: la separación clara entre frontend y backend para escalabilidad y mantenimiento, uso de DTOs para evitar exponer entidades directamente, implementación de patrón DAO para acceso a datos, autenticación basada en JWT con control de roles mediante Claims, uso de async/await para operaciones no bloqueantes y pruebas unitarias enfocadas en validación de entrada y respuestas de endpoints.
